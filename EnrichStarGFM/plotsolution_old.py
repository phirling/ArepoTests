...
def density_temperature_maps(fname, bins):
    with h5py.File(fname,'r') as f:
        ...
        ZAtom = ap.load_gas(f, 'GFM_Metallicity', remove_h_factors=0) / GFM_SOLAR_METALLICITY
    ...
    fields = [dens, temp*dens, ZAtom*dens]
...
def makeplot_maps(res, time_Myr, cmaps, labels, vmin=None, vmax=None):
    ...
    data = np.array(
        [
            [ np.log10(res[0]), np.log10(res[1]), np.log10(res[2]) ]
        ]
    )
    return niceplot.imshow_grid(1,3,data=data, s = 1.2, cmap=cmaps, label=labels,
...
if __name__ == '__main__':
    ...
    cmaps = [['cmr.ocean','magma', 'viridis']]
    vmin = [[-4.7 ,2.5 , 1e-9]]
    vmax = [[-4 ,4 , 4e-4]]
    labels = [[r'$\log_{10}\Sigma$ [g cm$^{-2}$]', r'$\log_{10}T$ [K]', 'ZAtom']]
