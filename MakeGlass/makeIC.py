...
# -------------------- Set Main Parameters
IC_filename = f"IC_Glass_N{N_CELLS_PER_DIM:n}.hdf5"
param_filename = "param.txt"
Boxsize = FloatType(BOXSIZE)
N = IntType(N_CELLS_PER_DIM)
dens = FloatType(NDENS)

if __name__ == '__main__':

    parser = argparse.ArgumentParser()
    parser.add_argument('--cosmo', action='store_true', help="Make IC for cosmological run")
    ...
    Mass = np.full(Npart_gas, mdens, dtype=FloatType)
    ...
    param_vals = [cell_mass] #, IC_filename]
